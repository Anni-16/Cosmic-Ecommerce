...
// Fetch vaastu details to delete associated images
$statement = $pdo->prepare("SELECT * FROM tbl_vaastu WHERE id=?");
$statement->execute([$_REQUEST['id']]);
...
// Delete the vaastu record itself
$statement = $pdo->prepare("DELETE FROM tbl_vaastu WHERE id=?");
$statement->execute([$_REQUEST['id']]);

header('location: Vaastu.php');
exit;
?>
